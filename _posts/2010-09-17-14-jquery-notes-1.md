...
</code></pre><em>除IE6/7的现代浏览器</em>都有<em>QuerySelectorAll</em>支持，能允许你传递一个像一样的CSS选择器，而不需要用到jQuery的内部代码。jQuery会在使用自己的引擎之前检查是否存在这个函数。
...
3.当使用Sizzle的时候，尽可能地把最右边的部分指明，比如<em>ID或者标签名</em><h2>3.不要滥用$(this)</h2>
...
});</code></pre><h2>4.jQuery的快速ready方法</h2>
原生的ready方法是这样的
...
</code></pre><h2>6.写聪明的代码</h2><pre><code>someDivs.each(function() {
